...
def stop_reason(tokenizer, response_tokens,response):
  if (
      (tokenizer.eos_token_id in response_tokens and 11822 in response_tokens) #final
      or ("Now we can" in response)
      or (tokenizer.eos_token_id in response_tokens and 'boxed' in response.lower())
  ):
      return "EOS"
...
class Particle:
    ...
    def deepcopy(self, numSteps=None):
        ...
        if numSteps is not None:
            ...
            new_particle.subq_list = self.subq_list[:numSteps]
            new_particle.suba_list = self.suba_list[:numSteps]
            ...
            new_particle.active = self.active
            new_particle.subq_list = self.subq_list
            new_particle.suba_list = self.suba_list            
